...
const StudentsAcademic = () => {
  ...
  // Filter students by name, admission number and class
  const filteredStudents = students.filter((stu) => {
    const search = searchTerm.toLowerCase();

    const matchSearch =
      stu.first_name?.toLowerCase().includes(search) ||
      stu.last_name?.toLowerCase().includes(search) ||
    ...
    const matchClass =
      selectedClass === "all" ||
      String(stu.current_class?.id) === selectedClass;
  ...
  const handleViewProfile = (studentId) => {
    navigate(`/academic-coordinator/students/${studentId}`);
  };
              ...
              <tbody>
                ...
                {filteredStudents.map((stu, idx) => (
                  ...
                  >
                    ...
                    {/* Current Class */}
                    <td className="py-3 px-3">
                      {stu.current_class?.grade ||
                        stu.current_class?.name ||
                        "Unassigned"}

                      {stu.current_class?.stream &&
                        ` - ${stu.current_class.stream}`}
                    </td>
