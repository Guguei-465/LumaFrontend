...
const ParentProfile = () => {
  ...
  // --- Password states (exact same as Teacher) ---
  ...
  // --- Fetch Profile — useCallback + handle nested data ---
  const fetchProfile = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const { data } = await api.get("accounts/parent-profile/");
      console.log("Raw Parent Profile data:", data); // ✅ matches your log
      // Safe array-or-object + set directly (preserves nested shape)
  ...
  const handleProfileChange = (e) => {
    ...
    if(["first_name","last_name","email","phone_number","role"].includes(name)){
      ...
      // Top-level fields: occupation, address, id
      setProfile(prev => ({ ...prev, [name]: value }));
    }
  };

  // --- Update Profile — ID in URL exactly like Teacher ---
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-gray-50 min-h-screen">
      {/* Header with Avatar — pass profile or profile.user to UserAvatar */}
      ...
      {/* Status messages — separate error/success EXACTLY like Teacher */}
      ...
      {/* --- Main Profile Info — correctly read nested + top-level --- */}
      <div className="card">
        {!editMode ? (
      ...
      {/* --- CHANGE PASSWORD — toggleable, exact same UI --- */}
      <div className="card">
